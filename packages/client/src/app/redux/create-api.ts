...
const createEndpointSlice = <E extends (...args: any) => any>(endpoint: E) => {
  ...
  const apiEndpointSlice = createSlice({
    ...
    reducers: {
      ...
      setData: (
        state,
        action: PayloadAction<Draft<Awaited<ReturnType<typeof endpoint>>>>
      ) => {
    ...
    reducer: Reducer<
      Record<
        ...
        ApiEndpointState<
          Awaited<
            ReturnType<
              EndpointsDefinitions[keyof EndpointsDefinitions]['initiate']
            >
          >
        >
      >
